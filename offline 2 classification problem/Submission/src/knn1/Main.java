            ...
            for(i2=0;i2<training_example.size();i2++)
            {
                ...
                while(items3.hasMoreElements())
                {
                    ...
                    if(training_example.get(i2).word_frequency.get(s)!=null)
                    {
                        ...
                        test_idf = Math.log10(409 / Word_occurence_documents_testing.get(s));
                        training_tf =(double) training_example.get(i2).word_frequency.get(s) / training_article_word_number;
                        training_idf = Math.log10((1192 / Word_occurence_documents_training.get(s)));
        ...
        try
        {
            ...
            while((s=textReader.readLine())!=null)
            {
               ...
               if(Topic_table.get(s.toString())!=null)
               {
                   ...
                   while(!(s2=textReader.readLine()).contains("REUTER"))
                   {
                       String [] tokens_string = s2.split(delim);
                       int i;
                       for(i=0;i<tokens_string.length;i++)
                       {
                           if(Word_table.get(tokens_string[i])!=null)
                           {
                               ...
                               if(word_frequency.get(Word_table.get(tokens_string[i]))==null)
                               {
                                   word_frequency.put(Word_table.get(tokens_string[i]),1);
                               }
                                else
                               {
                                   int x = word_frequency.get(Word_table.get(tokens_string[i]))+1;
                                   word_frequency.put(Word_table.get(tokens_string[i]),x);
                               }
                           }
                       }
                   }
